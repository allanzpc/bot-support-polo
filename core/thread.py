...
class Thread:
    ...
    def __init__(
        ...
        if isinstance(recipient, int):
            ...
            if recipient.bot:
                raise CommandError("Le destinataire ne peut pas être un bot...")
    ...
    async def setup(self, *, creator=None, category=None):
        ...
        try:
            channel = await self.bot.modmail_guild.create_text_channel(
                ...
                reason="Création d'un ticket de support",
            )
            ...
            embed.title = "Erreur lors de la création du ticket"
            embed.description = str(e)
            embed.add_field(name="Destinataire", value=recipient.mention)
    ...
    def _format_info_embed(self, user, log_url, log_count, color):
        ...
        if member is None or len(mutual_guilds) > 1:
            embed.add_field(
                name="Serveurs en commun (bot - user)", value=", ".join(g.name for g in mutual_guilds)
            )
    ...
    async def find_linked_messages(
        ...
        async for msg in self.recipient.history():
            ...
            try:
                ...
                continue
        raise ValueError("Message privé non trouvé")
        ...
        if self.close_task is not None:
            ...
            self.bot.loop.create_task(
                self.channel.send(
                    embed=discord.Embed(
                        color=self.bot.error_color,
                        description="La fermeture à été annulée avec succès",
                    )
                )
            )
