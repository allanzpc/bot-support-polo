...
class Modmail(commands.Cog):
    ...
    async def setup(self, ctx):
        ...
        if ctx.guild != self.bot.modmail_guild:
            return await ctx.send(
                f"Vous ne pouvez utiliser cette commande uniquement dans le serveur enregistré dans la BDD : {self.bot.modmail_guild}."
            )

        if self.bot.main_category is not None:
            logger.debug("Vous ne pouvez pas réutiliser cette commande... \n La catégorie à déjà été crée")
            return await ctx.send(f"{self.bot.modmail_guild} est déjà setup.")

        if self.bot.modmail_guild is None:
            embed = discord.Embed(
                title="Erreur",
                description="Serveur de référence non trouvé...",
        ...
        embed.add_field(
            name="Merci d'utiliser notre bot !",
            value="Ce bot est un bot Discord de support.\n Les membres du serveur peuvet lui envoyer un message pour \n demander de l'aide du Staff."
        )

        embed.set_footer(text=f'Utilisez "{self.bot.prefix}help" pour voir la liste complète des commandes')
        ...
        await ctx.send(
            "**Setup reussi !**\n"
            "Les utilisateurs pouvant voir les tickets sont : "
            "Les administrateurs du serveur et les développeurs du bot.\n\n"
    ...
    async def snippet(self, ctx, *, name: str.lower = None):
        ...
        if not self.bot.snippets:
            embed = discord.Embed(
                color=self.bot.error_color, description="Vous n'avez pas de snippets pour le moment."
            )
            embed.set_footer(text=f'Faites "{self.bot.prefix}help snippet add" pour ajouter un snippet.')
    ...
    async def snippet_add(self, ctx, name: str.lower, *, value: commands.clean_content):
        ...
        if name in self.bot.snippets:
            embed = discord.Embed(
                title="Erreur",
                color=self.bot.error_color,
                description=f"Le snippet : `{name}` existe déjà... .",
            )
            return await ctx.send(embed=embed)
        ...
        if len(name) > 120:
            embed = discord.Embed(
                title="Erreur",
                color=self.bot.error_color,
                description="Le noms des snippets ne peuvent pas dépasser les 120 caractères de long",
            )
            return await ctx.send(embed=embed)
        ...
        embed = discord.Embed(
            title="Snippet crée",
            color=self.bot.main_color,
            description="Le snippet à été crée avec succès !",
        )
        return await ctx.send(embed=embed)
    ...
    async def snippet_remove(self, ctx, *, name: str.lower):
        ...
        if name in self.bot.snippets:
            embed = discord.Embed(
                ...
                color=self.bot.main_color,
                description=f"Le snippet `{name}` est maintenat supprimé.",
            )
    ...
    async def snippet_edit(self, ctx, name: str.lower, *, value):
        ...
        if name in self.bot.snippets:
            ...
            embed = discord.Embed(
                title="Snippet édité",
                color=self.bot.main_color,
                description=f'`{name}` est maintenant "{value}".',
            )
        else:
            embed = create_not_found_embed(name, self.bot.snippets.keys(), "Snippet")
        await ctx.send(embed=embed)
    ...
    async def send_scheduled_close_message(self, ctx, after, silent=False):
        ...
        embed = discord.Embed(
            title="Ticket fermé avec succès",
            description=f"Icket fermé {silent}par {human_delta}.",
            color=self.bot.error_color,
        )
        ...
        embed.set_footer(text="La fermeture sera annulée si un message est envoyé.")
    ...
    async def notify(
        ...
        if mention in mentions:
            ...
            embed = discord.Embed(
                color=self.bot.main_color,
                description=f"{mention} sera mentionné aux prochains messages envoyés",
            )
        return await ctx.send(embed=embed)
    ...
    async def subscribe(
        ...
        if mention in mentions:
            embed = discord.Embed(
                color=self.bot.error_color,
                description=f"{mention} à déjà souscris à ce ticket.",
            )
        else:
            mentions.append(mention)
            await self.bot.config.update()
            embed = discord.Embed(
                color=self.bot.main_color,
                description=f"{mention} sera notifié à chaque message reçu.",
            )
        return await ctx.send(embed=embed)
    ...
    def format_log_embeds(self, logs, avatar_url):
        ...
        for entry in logs:
            ...
            embed.add_field(name="Fermé par", value=closer_msg)

            if entry["recipient"]["id"] != entry["creator"]["id"]:
                embed.add_field(name="Crée par", value=f"<@{entry['creator']['id']}>")
            ...
            embed.set_footer(text="ID du destinataire: " + str(entry["recipient"]["id"]))
            embeds.append(embed)
        return embeds
    ...
    async def maintenance_off(self, ctx):
        ...
        embed = discord.Embed(
            ...
            description="**Le mode maintenance est maintenant désactivé**. \n *Les utilisateurs peuvent m'envoyer des messages privés.*",
            color=self.bot.main_color,
        )
    ...
    async def disable_new(self, ctx):
        ...
        embed = discord.Embed(
            ...
            description="Le mode maintenance est maintenant activé.",
            color=self.bot.main_color,
        )
        if self.bot.config["dm_disabled"] < 1:
    ...
    async def maintenance_on(self, ctx):
        ...
        embed = discord.Embed(
            title="Mode maintenance activé",
            description="Vous venez d'activer le mode maintenance. \n Les membres ne peuvent plus m'envoyer de messages privés.",
            color=self.bot.main_color,
        )
    ...
    async def maintenance_mode(self, ctx):
        ...
        if self.bot.config["dm_disabled"] == 1:
            embed = discord.Embed(
                title="Activé",
                description="Le mode maintenance est activé, les membres ne peuvent pas m'envoyer de MP.",
                color=self.bot.error_color,
            )
            ...
            embed = discord.Embed(
                title="Désactivé",
                description="Le mode maintenance est désactivé, les utilisateurs peuvent m'envoyer des MP",
                color=self.bot.main_color,
            )
